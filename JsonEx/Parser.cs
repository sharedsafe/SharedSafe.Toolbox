...
namespace JsonExSerializer
{
    sealed class Parser
    {
        ...
        public object Parse()
        {
			...
			// armin: we need the test if the deserialized type is compatible if result type can not be set
			if (!expr.SetResultTypeIfNotSet(_deserializedType) && !_deserializedType.IsAssignableFrom(expr.ResultType))
				throw new ParseException(String.Format(
					"Parsed result type {0} is not compatible with expected type {1}", expr.ResultType,
						_deserializedType));
		...
		Type bindType(string typeName)
		{
			return _context.GetTypeBinding(typeName) ?? Type.GetType(typeName, true);
		}
