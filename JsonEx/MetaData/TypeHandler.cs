...
namespace JsonExSerializer.MetaData
{
    ...
    public class TypeHandler : MemberHandlerBase
    {
		...
		//armin
		SchemaEvolutionAttribute[] _schemaEvolutions;
        ...
        public virtual object CreateInstance(object[] args)
        {
			...
			if (args.Length != 0 || HasDefaultConstructor)
				return Activator.CreateInstance(ForType, args);

			return FormatterServices.GetUninitializedObject(ForType);
        }
		...
		bool HasDefaultConstructor
		{
			get
			{
				if (!_hasDefaultConstructor.HasValue)
					_hasDefaultConstructor = ForType.GetConstructor(Type.EmptyTypes) != null;
				return _hasDefaultConstructor.Value;
			}
		}
        ...
        public AbstractPropertyHandler FindProperty(string Name)
        {
			...
			// armin (integration of schema evolution)

			if (_schemaEvolutions == null)
				_schemaEvolutions = _forType.getCustomAttributesClosure<SchemaEvolutionAttribute>().ToArray();

			foreach (var se in _schemaEvolutions)
			{
				var me = se.queryMemberEvolution(_forType, Name);
				// todo: cache

				if (me != null)
					return new MemberEvolutionPropertyHandler(_forType, me.Value);
			}
		...
		sealed class MemberEvolutionPropertyHandler : AbstractPropertyHandler
		{
			readonly MemberEvolution _me;

			public MemberEvolutionPropertyHandler(Type forType, MemberEvolution me)
				: base(forType)
			{
				_me = me;
			}
