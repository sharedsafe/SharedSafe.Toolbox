				...
				if (_offset == _end)
					if (!readIn())
					{
						if (_state != EOF && _state != 0)
							throw new Exception("invalid end of file, base64 decoding state {0}".format(_state));
				...
				if (decoded == Padding)
				{
					switch (_state)
					{
						...
						case 1: ///< padding after six bits, too (partial bytes are not supported)
							throw new Exception("invalid base64 decoding state {0} when received padding, data malformed".format(_state));
